...
const ChatPage: React.FC = () => {
  ...
  // Handle follow-up from report page: ?stock=600519&name=貴州茅台&recordId=xxx
  useEffect(() => {
  ...
  const downloadMessageAsMarkdown = useCallback((msg: Message) => {
    ...
    const heading = msg.role === 'user' ? '# 用户消息' : `# AI 回覆${skillLabel ? ` · ${skillLabel}` : ''}`;
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title="開始問股"
                  description="輸入「分析 600519」或「茅台現在能買嗎」，AI 將調用實時數據工具為您生成決策報告。"
            ...
            {skills.length > 0 && (
              ...
              <div className="flex items-end gap-3">
                <textarea
                  ...
                  placeholder="例如：分析 600519 / 茅台現在適合買入嗎？ (Enter 發送, Shift+Enter 換行)"
                  disabled={loading}
                  rows={1}

            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-4">
                <Input
                  ...
                  allowTogglePassword
                  iconType="password"
                  label={isFirstTime ? '管理員密碼' : '登錄密碼'}
                  placeholder={isFirstTime ? '請設置 6 位以上密碼' : '請輸入密碼'}
                  value={password}
                ...
                <div className="relative z-10 flex items-center justify-center gap-2">
                  {isSubmitting ? (
                    <>
                      ...
                      <span>{isFirstTime ? '初始化中...' : '正在建立連接...'}</span>
                    </>
                  ) : (
                    <span>{isFirstTime ? '完成設置並登錄' : '授權進入工作台'}</span>
                  )}
                </div>

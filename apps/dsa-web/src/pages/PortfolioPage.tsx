...
const PortfolioPage: React.FC = () => {
  ...
  const handleTradeSubmit = async (e: React.FormEvent) => {
    ...
    if (!writableAccountId) {
      setWriteWarning('請先在右上角選擇具體賬户，再進行錄入或導入提交。');
      return;
    }
    try {
      setWriteWarning(null);
  ...
  const openDeleteDialog = (item: PendingDelete) => {
    if (!writableAccountId) {
      setWriteWarning('請先在右上角選擇具體賬户，再進行刪除修正。');
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    if (!name) {
      setAccountCreateError('賬户名稱不能為空。');
    ...
    try {
      ...
      setAccountCreateSuccess('賬户創建成功，已自動切換到該賬户。');
    } catch (err) {
      const parsed = getParsedApiError(err);
      setAccountCreateError(parsed.message || '創建賬户失敗，請稍後重試。');
      setAccountCreateSuccess(null);
    } finally {
    ...
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
      <section className="space-y-3">
        <div className="space-y-2">
          ...
          <p className="text-xs md:text-sm text-secondary">
            組合快照、手工錄入、CSV 導入與風險分析（支持全組合 / 單賬户切換）
          </p>
        </div>
        {hasAccounts ? (
          ...
          <InlineAlert
            ...
            message="還沒有可用賬户，請先創建賬户後再錄入交易或導入 CSV。"
          />
        )}
      </section>
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-foreground">新建賬户</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              <span className="text-xs text-secondary">創建後自動切換到該賬户</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card className="xl:col-span-2" padding="md">
          ...
          {positionRows.length === 0 ? (
            <EmptyState
              title="當前無持倉數據"
              description="錄入交易或導入 CSV 後，這裏會展示按賬户彙總的持倉明細。"
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="text-xs text-secondary border-b border-white/10">
                  <tr>
                    <th className="text-left py-2 pr-2">賬户</th>
                    <th className="text-left py-2 pr-2">代碼</th>
        ...
        <Card padding="md">
          <h2 className="text-sm font-semibold text-foreground mb-3">{concentrationMode === 'sector' ? '行業集中度分佈' : '行業數據暫不可用，當前展示個股集中度'}</h2>
          {concentrationPieData.length > 0 ? (
            ...
            <EmptyState
              title="暫無集中度數據"
              description="風險模塊完成計算後，這裏會展示行業或個股維度的集中度分佈。"
      ...
      {writeBlocked && hasAccounts ? (
        <InlineAlert
          ...
          message="當前處於“全部賬户”視圖。為避免誤寫，請先選擇一個具體賬户後再進行手工錄入或 CSV 提交。"
        />
      ) : null}
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-3">
        ...
        <Card padding="md">
          ...
          <div className="text-xs text-secondary space-y-1">
            <div>賬户數: {snapshot?.accountCount ?? 0}</div>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <input className={PORTFOLIO_INPUT_CLASS} type="number" min="0" step="0.0001" placeholder="税費（可選）" value={tradeForm.tax}
                onChange={(e) => setTradeForm((prev) => ({ ...prev, tax: e.target.value }))} />
            </div>
            <p className="text-xs text-secondary">手續費和税費可留空，系統將按 0 處理。</p>
        ...
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            ...
            <input className={PORTFOLIO_INPUT_CLASS} placeholder={`幣種（可選，默認 ${writableAccount?.baseCurrency || '賬户基準幣'}）`} value={cashForm.currency}
        ...
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            <div className="text-[11px] text-secondary">
              {writeBlocked ? '刪除修正僅在單賬户視圖可用。請先選擇具體賬户後再刪除錯誤流水。' : '如有錯誤流水，可直接刪除後重新錄入。'}
            </div>

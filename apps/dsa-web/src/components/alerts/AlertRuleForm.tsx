...
const TARGET_SCOPE_OPTIONS = [
  ...
  { value: 'portfolio_account', label: '持倉賬户' },
];
...
export const AlertRuleForm: React.FC<AlertRuleFormProps> = ({ onSubmit, isSubmitting = false }) => {
  ...
  useEffect(() => {
    ...
    void portfolioApi.getAccounts(false)
      ...
      .catch((error: unknown) => {
        if (cancelled) return;
        setAccounts([]);
        setAccountsError(error instanceof Error ? error.message : '賬户加載失敗');
      });
    return () => {
      cancelled = true;
    };
  ...
  const ensureRequiredBarsWithinLimit = (label: string, requiredBars: number): boolean => {
    if (requiredBars > MAX_REQUESTED_DAYS) {
      setFormError(`${label} 週期組合需要 ${requiredBars} 根日線，最多支持 ${MAX_REQUESTED_DAYS} 根`);
      return false;
    }
    return true;
  };
  ...
  const buildParameters = (): AlertRuleCreateRequest['parameters'] | null => {
    ...
    if (alertType === 'macd_cross') {
      ...
      const parsedSignal = parseIntegerInRange(signalPeriod, '信號週期');
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (targetScope === 'single_symbol') {
      ...
      if (!targetValidation.valid) {
        setFormError(targetValidation.message ?? '股票代碼格式不正確');
        return;
      }
  ...
  return (
    <Card title="創建告警規則" subtitle="Web 告警中心" variant="bordered" padding="md">
      <form className="space-y-4" noValidate onSubmit={(event) => void handleSubmit(event)}>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Checkbox
            ...
            disabled={isSubmitting}
          />
          <Button type="submit" isLoading={isSubmitting} loadingText="創建中...">
            創建規則
          </Button>
        </div>

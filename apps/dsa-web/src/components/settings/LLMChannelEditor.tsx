...
const LLM_REASON_HINTS: Record<string, string> = {
  ...
  insufficient_balance: '服務商返回餘額、賬單或額度不足；請檢查賬户餘額和套餐狀態。',
  ...
  const handleSave = async () => {
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidAgentPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: '當前 Agent 主模型不在已啓用渠道的模型列表中，請重新選擇。' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: '當前 Vision 模型不在已啓用渠道的模型列表中，請重新選擇。' });
        return;
      }
    }
            ...
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  先添加至少一個已啓用渠道並填寫模型，下面的主模型 / 備選模型 / Vision 選項才會出現。
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(availableModels, runtimeConfig.visionModel, '自動（跟隨 Vision 默認邏輯）')}
                      disabled={busy}
                      placeholder=""
                    />
                  </div>

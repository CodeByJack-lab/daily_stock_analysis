...
function createNextModeLabel(authEnabled: boolean, desiredEnabled: boolean) {
  ...
  return authEnabled ? '保持已開啓' : '保持已關閉';
}
...
export const AuthSettingsCard: React.FC = () => {
  ...
  const helperText = useMemo(() => {
    switch (setupState) {
      case 'no_password':
        return '系統尚未設置密碼。啓用認證前請先設置初始管理員密碼，設置後請妥善保管。';
      case 'password_retained':
        return '系統已保留之前設置的管理員密碼。輸入當前密碼即可快速重新啓用認證。';
      case 'enabled':
        return !desiredEnabled 
          ? '若當前登錄會話仍有效，可直接關閉認證；若會話已失效，請輸入當前管理員密碼。'
          : '管理員認證已啓用。如需更新密碼，請使用下方的“修改密碼”功能。';
      default:
        return '管理員認證可保護 Web 設置頁及 API 接口，防止未經授權的訪問。';
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      await refreshStatus();
      setSuccessMessage(desiredEnabled ? '認證設置已更新' : '認證已關閉');
      resetForm();
    } catch (err: unknown) {
      ...
      <form className="space-y-4" onSubmit={handleSubmit}>
             ...
             (setupState === 'enabled' && !desiredEnabled) ? (
              <div className="space-y-3">
                <Input
                  ...
                  disabled={isSubmitting}
                  placeholder="請輸入當前密碼"
                  hint={setupState === 'password_retained' ? '輸入舊密碼以重新激活認證' : '關閉認證前可能需要驗證身份'}
                />
              </div>
            ) : null}
        ...
        {error ? (
          isParsedApiError(error) ? (
            ...
            <SettingsAlert title="認證設置失敗" message={error} variant="error" />
          )
        ) : null}

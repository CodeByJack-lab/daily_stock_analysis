  ...
  const handleImageFile = useCallback(
    async (file: File) => {
      ...
      if (!IMG_EXT.includes(ext)) {
        setError('圖片僅支持 JPG、PNG、WebP、GIF');
        return;
      }
      ...
      try {
        ...
        else if (err?.code === 'ECONNABORTED') fallback = '請求超時，請檢查網絡後重試';
  ...
  const mergeToWatchlist = useCallback(async () => {
    ...
    try {
      ...
      if (e instanceof SystemConfigConflictError) {
        await onMerged(value);
        setError('配置已更新，請再次點擊「合併到自選股」');
      } else {
        setError(e instanceof Error ? e.message : '合併保存失敗');
      }
    } finally {
      setIsMerging(false);
    }
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <textarea
            placeholder="或粘貼 CSV/Excel 複製的文本..."
      ...
      {items.length > 0 && (
        <div className="space-y-2">
          <InlineAlert
            variant="warning"
            message="建議人工逐條核對後再合併。高置信度默認勾選，中/低置信度需手動確認。"
